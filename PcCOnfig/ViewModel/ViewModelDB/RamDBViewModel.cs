...
{
    class RamDbViewModel : ConfigDbComponentsCommonPresenter
    {
        ...
        public RamDbViewModel()
        {
            using (var db = new ComponentContext())
            {
                var res = db.Rams.Cast<Ram>().Where(x => x.IsDeleted == false);
        ...
        protected override void Add()
        {
            ...
            RamConnectionEnum connection;
            if (!Enum.TryParse(ConnectionType, out connection))
            {
                return;
            }
            ram.ConnectionType = connection;
